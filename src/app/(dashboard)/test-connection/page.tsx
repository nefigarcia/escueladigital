...
import { CheckCircle2, XCircle, Loader2, Database, ShieldCheck, Cpu } from "lucide-react"
...
export default function TestConnectionPage() {
  ...
  const { firebaseApp, auth, areServicesAvailable } = useFirebase()
      ...
      <Card>
        ...
        <CardContent className="space-y-4 text-sm">
          <ul className="list-disc pl-5 space-y-2 text-muted-foreground">
            ...
            <li>
              <strong className="text-foreground">Reglas de Seguridad:</strong> El sistema está configurado para requerir roles de Staff. Si no has creado un documento en <code className="bg-muted px-1 rounded">/staff_roles/{auth?.currentUser?.uid}</code> con el campo <code className="bg-muted px-1 rounded">role: "Administrador"</code>, las lecturas serán denegadas.
            </li>
            <li>
              <strong className="text-foreground">Firestore Habilitado:</strong> Verifica que hayas creado la base de datos en la consola de Firebase y seleccionado "Modo de Producción" o "Modo de Prueba".
            </li>
          </ul>
        </CardContent>
      </Card>
